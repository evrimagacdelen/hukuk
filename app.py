# ==============================================================================
# BU KOD, İSTEDİĞİNİZ TÜM DEĞİŞİKLİKLERİ İÇEREN NİHAİ VERSİYONDUR.
# ==============================================================================
...
# 🚨 UYARI: Bu anahtarı ASLA herkese açık bir GitHub deposuna göndermeyin!
try:
    # ÖNCEKİ MESAJLARDA SIZDIRDIĞINIZ ANAHTARI DEĞİL, YENİ OLUŞTURDUĞUNUZ GÜVENLİ ANAHTARI KULLANIN!
...
if models_bundle is None or df_data is None:
    st.warning("Modeller veya veri dosyası yüklenemedi. Uygulama durduruldu.")
    st.stop()
...
def predict_case(text, law_vec, damage_vec, law_mdl, damage_mdl, classes):
    ...
    return predicted_laws, "VAR" if damage_prediction_code == 1 else "YOK"
...
def get_gemini_summary(text):
    if not gemini_model: return "Gemini modeli yüklenemediği için özet oluşturulamadı."
    try:
        prompt = f'Aşağıdaki hukuki metni analiz et ve ana konuyu, tarafların argümanlarını ve sonucu vurgulayan, herkesin anlayabileceği dilde kısa bir özet çıkar.\n\nMetin:\n"{text}"\n\nÖzet:'
...
with col1:
    ...
    input_text = st.text_area("Analiz edilecek metnin başlangıç kısmını buraya girin:", height=300, placeholder="Örnek: Eşi çalışan personele aile yardımı ödeneği ödenmesi...")
    ...
    if st.button("🔍 Analiz Et", type="primary", use_container_width=True):
        if not input_text.strip():
            ...
            with st.spinner("Analiz yapılıyor..."):
                ...
                st.session_state.update({'predicted_laws': laws, 'predicted_damage': damage})
                full_text = find_full_text(df_data, input_text)
                st.session_state['gemini_summary'] = get_gemini_summary(full_text) if full_text else "Girdiğiniz metinle eşleşen bir 'Tam Metin' Excel dosyasında bulunamadı."
